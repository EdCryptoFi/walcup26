  ...
  return (
    <div className={`sticker-card ${tilt} peel-corner rounded-xl overflow-hidden`}>
      <div className={`px-5 py-4 border-b border-outline-variant flex items-center justify-between`}>
        <span className={`pill ${labelColor} font-black text-base px-3 py-1`}>Group {group}</span>
        <span className="text-xs text-on-surface-variant">{playedMatches.length}/{matches.length} played</span>
      </div>
      ...
      <div className="px-4 py-3">
        <div className="grid grid-cols-[1fr_24px_24px_24px_24px_32px] gap-1 text-[11px] text-on-surface-variant px-1 mb-1.5 font-bold uppercase">
          <span>Team</span>
        ...
        {sorted.map((s, i) => {
          ...
          return (
            <div
              key={s.teamId}
              className={`grid grid-cols-[1fr_24px_24px_24px_24px_32px] gap-1 items-center text-sm px-1 py-2 rounded-lg ${advancing ? 'bg-pitch-green/10' : ''}`}
            >
              <div className="flex items-center gap-2 min-w-0">
                {advancing && <span className="w-1 h-4 bg-pitch-green rounded-full flex-shrink-0" />}
                <span className="text-base">{team?.flag}</span>
                <span className={`truncate font-semibold ${advancing ? 'text-on-surface' : 'text-on-surface-variant'}`}>{team?.code}</span>
              </div>
              <span className="text-center text-on-surface-variant">{s.played}</span>
              <span className="text-center text-on-surface">{s.won}</span>
              <span className="text-center text-on-surface-variant">{s.drawn}</span>
              <span className="text-center text-on-surface-variant">{s.lost}</span>
              <span className="text-center font-black text-primary text-base">{s.pts}</span>
            </div>
          );
        })}
      </div>
          ...
          {playedMatches.map((m) => {
            ...
            return (
              <div key={m.id} className="flex items-center text-sm gap-2 bg-surface-container rounded-lg px-2 py-1.5">
                ...
                <span className="font-black text-on-surface tabular-nums px-2">{res.homeScore}–{res.awayScore}</span>
                <span className="flex-1 text-right text-on-surface-variant truncate">{away?.code} {away?.flag}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-black text-on-surface">Group Stage</h1>
          <p className="text-on-surface-variant text-sm mt-0.5">48 teams · 12 groups · June 11–27, 2026</p>
        </div>
        <div className="flex gap-3 text-xs text-on-surface-variant items-center">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 bg-pitch-green rounded-full" />
            Advances to Round of 32
          </span>
          <Link href="/predict" className="bg-primary text-white rounded-full px-4 py-2 font-bold text-xs hover:scale-105 transition-all">
            Make Predictions →
          </Link>
        </div>
      </div>
      ...
      <div className="sticker-card sticker-tilt-1 rounded-xl p-4 text-sm text-on-surface-variant" style={{ background: 'rgba(0,74,198,0.04)' }}>
        <strong className="text-on-surface">Format:</strong> Top 2 from each group + 8 best 3rd-place teams → Round of 32 (40 teams total). 72 group stage matches across USA, Canada, and Mexico.
      </div>
    </div>
  );
}
